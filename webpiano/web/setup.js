/* PiMaestro Setup screen: the per-song configuration VIEW — the Part dropdown (which part you play),
   ...
   outside is an injected function:
     ...
     OUTPUTS : onPlay(channels) — "the player re-picked which tracks they play".
...
const PiSetup = (function () {
  ...
  const handSel = $('handSel'), modeSel = $('modeSel'), modeHint = $('modeHint'),
        instrPanel = $('instrPanel'), favBtn = $('fav');
  let ctx = {
    getVM: () => null, getPlay: () => [], getSelFile: () => null,
    control: () => Promise.resolve(), onPlay: () => {},
