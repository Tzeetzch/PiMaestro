...
- `noteon`/`noteoff` — the player's live keys (for the on-screen keyboard, the staff overlay, browser sound).
- `autoon`/`autooff`/`alloff` — accompaniment the conductor is playing, mirrored so a browser can sonify it.
  **Best-effort deltas**, not guaranteed: the browser bounds each voice's length so a dropped `autooff`
  self-releases instead of ringing forever (see app.js voice pools). Drums (`ch 9`) are skipped by the browser.
- `rating` — instant early/good/late feedback per chord.

## Sound

- **Authoritative sound is the Pi's FluidSynth.** Live keys go keyboard→ALSA→FluidSynth (no web round-trip),
  so latency is synth+buffer only. Accompaniment is sent over the TCP command server (:9800).
...
- **Browser sound (WebAudioFont)** is a *remote-listening convenience* for a tablet/laptop — never
  sample-accurate with the Pi. Leave it OFF on the Pi/TV. Single grand-piano preset today; a per-GM-program
  preset bank is a known follow-up (needs bundling more preset assets).
...
- Per-GM-program WebAudioFont preset bank for browser sound (assets).
